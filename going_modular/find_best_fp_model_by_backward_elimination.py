...
from .test_fp_model_with_cross_validation import test_fp_model_with_cross_validation

def find_best_fp_model_by_backward_elimination(train_df, covariates, df_candidates, n_splits=5):
    ...
    while True:
        ...
        for df_spline in df_candidates:
            score = test_fp_model_with_cross_validation(train_df, current_vars, df_spline, n_splits)
            candidates.append((score, current_vars, df_spline))

        for var in current_vars:
            test_vars = [v for v in current_vars if v != var]
            for df_spline in df_candidates:
                score = test_fp_model_with_cross_validation(train_df, test_vars, df_spline, n_splits)
