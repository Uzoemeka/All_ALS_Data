...
def prediction_survival_for_average_patient_cat_cov_fp_model_train_test(
    df_train,
    df_test,
    df_valid,
    fp_model,
    ...
    if newdata_0_valid is not None:
        S0_valid = r_predict(fp_model, newdata=newdata_0_valid, type="surv")
        with localconverter(default_converter + pandas2ri.converter):
    ...
    if newdata_1_valid is not None:
        S1_valid = r_predict(fp_model, newdata=newdata_1_valid, type="surv")
        with localconverter(default_converter + pandas2ri.converter):
